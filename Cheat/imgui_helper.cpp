...
void ImGuiHelper::drawTabHorizontally(std::string childName, ImVec2 childSize, std::vector<std::string>tabNames, int& selectedSubTab) {
	...
	ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding, 0); // round buttons
	for (int i = 0; i < tabNames.size(); i++) {
		...
		ImGui::PushStyleColor(ImGuiCol_Text, selectedSubTab == i ? ImGui::GetStyle().Colors[ImGuiCol_Text] : col2);
